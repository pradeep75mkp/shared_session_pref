...
class SharedSessionPref {
  SharedSessionPref._(this._preferenceCache);

  static String _prefix = 'flutter.mkp.';

  static bool _prefixHasBeenChanged = false;

  static Set<String>? _allowList;

  static Completer<SharedSessionPref>? _completer;

  static SharedSessionPrefPlatform get _store =>
      SharedSessionPrefPlatform.instance;
  ...
  /// Loads and parses the [SharedSessionPref] for this app from disk.
  ///
  /// Because this is reading from disk, it shouldn't be awaited in
  /// performance-sensitive blocks.
  static Future<SharedSessionPref> getInstance() async {
    if (_completer == null) {
      final Completer<SharedSessionPref> completer =
      Completer<SharedSessionPref>();
      _completer = completer;
      try {
        final Map<String, Object> preferencesMap =
        await _getSharedPreferencesMap();
        completer.complete(SharedSessionPref._(preferencesMap));
      } catch (e) {
        // If there's an error, explicitly return the future with an error.
        // then set the completer to null so we can retry.
        completer.completeError(e);
        final Future<SharedSessionPref> sharedPrefsFuture = completer.future;
        _completer = null;
...
Either update the implementation to support setPrefix, or do not call setPrefix.
  ...
  Future<void> reload() async {
    ...
    await SharedSessionPref._getSharedPreferencesMap();
    _preferenceCache.clear();
